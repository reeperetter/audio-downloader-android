...
async def main(page: ft.Page):
    ...
    search_input = ft.TextField(
        hint_text="Наприклад: Marshroot Незавершена",
        expand=True,
    )
    ...
    select_all_btn = ft.OutlinedButton("Вибрати все")
    clear_btn = ft.OutlinedButton("Зняти вибір")
    download_btn = ft.ElevatedButton("Завантажити MP3", disabled=True)
        ...
        try:
            ...
            if stream_url:
                ...
                page.snack_bar = ft.SnackBar(
                    ft.Text("Не вдалося отримати аудіопотік."))
                page.snack_bar.open = True
                await page.update_async()
        except Exception as exc:
            play_btn.icon = ft.icons.PLAY_ARROW
            page.snack_bar = ft.SnackBar(
                ft.Text(f"Помилка відтворення: {exc}"))
            page.snack_bar.open = True
            await page.update_async()
    ...
    async def start_search(e):
        ...
        try:
            ...
            if search_results_data:
                status_label.value = f"Знайдено: {len(search_results_data)} результатів."
    ...
    async def start_download(e):
        ...
        def _progress_hook(d):
            if d["status"] == "downloading":
                ...
                status_label.value = "Конвертація в MP3..."
                asyncio.run_coroutine_threadsafe(page.update_async(), loop)

        def _download_sync():
            ydl_opts = {
                "format": "bestaudio/best",
                "outtmpl": os.path.join(selected_folder, "%(uploader)s - %(title)s.%(ext)s"),
                "postprocessors": [{
        ...
        try:
            ...
            page.snack_bar = ft.SnackBar(
                ft.Text("Усі треки успішно завантажено!"))
            page.snack_bar.open = True
        except Exception as exc:
                ...
                ft.Card(
                    content=ft.Container(
                        content=ft.Column([
                            ...
                            ft.Row([select_all_btn, clear_btn,
                                   ft.Container(expand=True), download_btn]),
                        ]),
                        padding=10,
                    )
                ),
